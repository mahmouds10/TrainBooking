...
{
    public partial class Delete_Trip : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            using (SqlConnection connection = new SqlConnection(coniction_st))
            {
                ...
                command.Parameters.AddWithValue("@id", Trip_ID.Text);
                connection.Open();
                command.ExecuteNonQuery();
                SqlCommand countCommand = new SqlCommand("SELECT COUNT(*) FROM Trip", connection);
                int rowCount = (int)countCommand.ExecuteScalar();
                SqlCommand reseedCommand = new SqlCommand($"DBCC CHECKIDENT ('Trip', RESEED, {rowCount})", connection);
        ...
        private void Delete_Trip_Load(object sender, EventArgs e)
        {
